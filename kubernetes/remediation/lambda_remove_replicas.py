...
def lambda_handler(event, context):
    ...
    remove_replicas = event.get("remove_replicas", 1)
    response_steps = []
    ...
    try:
        ...
        new_replicas = max(1, current_replicas - remove_replicas)  # Ensure at least 1 replica remains
        step_detail["request"] = f"Decrease replicas of '{replica_set_name}' by {remove_replicas} for pod '{pod_name}' with operation ID '{operation_id}'."
