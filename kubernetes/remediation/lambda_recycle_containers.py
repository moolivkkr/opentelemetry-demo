...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Step 2: Scale Up the Deployment to 2x current replicas
        step_detail = {"step": "scale_up", "request_id": request_id}
        step_detail["request"] = f"Increasing replicas for deployment '{deployment_name}' in namespace '{namespace}' by doubling the current number of replicas."
        deployment = v1_apps.read_namespaced_deployment(name=deployment_name, namespace=namespace)
        current_replicas = deployment.spec.replicas
        new_replicas = current_replicas * 2  # Scale to 2x
        ...
        # Wait for the new pods to become healthy
        new_pod_names = wait_for_new_pods(namespace, deployment_name, current_replicas, v1_core)

        # Step 3: Scale Down the Deployment to 1x original replicas
        ...
        deployment.spec.replicas = current_replicas  # Scale back down to 1x
        v1_apps.patch_namespaced_deployment(name=deployment_name, namespace=namespace, body=deployment)
        ...
        response_steps.append({
            ...
            "response": f"New pods created: {new_pod_names}"
        })
