...
def lambda_handler(event, context):
    ...
    scale_type = event.get("scale_type", "increase")  # increase or decrease
    scale_by = event.get("scale_by", 25)  # percentage or fixed number
    is_percentage = event.get("is_percentage", True)  # if True, scale_by is treated as percentage
    response_steps = []
    ...
    try:
        ...
        scale_direction = 1 if scale_type == "increase" else -1
        if is_percentage:
            scale_amount = int(current_replicas * (scale_by / 100)) * scale_direction
        else:
            scale_amount = scale_by * scale_direction
        new_replicas = max(1, current_replicas + scale_amount)  # Ensure at least one replica
        step_detail["request"] = f"Scale {scale_type} by {scale_by}{'%' if is_percentage else ''} for '{deployment_name}'."
        step_detail["response"] = f"Calculated new replica count: {new_replicas}."
        response_steps.append(step_detail)
        ...
        response_steps.append({
            ...
            "scale_by": f"{scale_by}{'%' if is_percentage else ''}",
            "response": f"Smart scaling applied to '{deployment_name}': scaled {scale_type} to {new_replicas} replicas."
        })
